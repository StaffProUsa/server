...
public class StaffUsuario {
    ...
    public static void onMessage(JSONObject obj, SSSessionAbstract session) {
        switch (obj.getString("type")) {
            ...
            case "aprobar": aprobar(obj, session); break;
            case "desaprobar": desaprobar(obj, session); break;
            case "asignarJefe": asignarJefe(obj, session); break;
            case "asignarJefeMultiple": asignarJefeMultiple(obj, session); break;
            case "editar": editar(obj, session); break;
            case "editarClock": editarClock(obj, session); break;
            case "invitar": invitar(obj, session); break;
            case "invitarGrupo": invitarGrupo(obj, session); break;
            case "desinvitarGrupo": desinvitarGrupo(obj, session); break;
            case "getInvitacionesPendientes": getInvitacionesPendientes(obj, session); break;
            case "aceptarInvitacion": aceptarInvitacion(obj, session); break;
            case "getTrabajosProximos": getTrabajosProximos(obj, session); break;
            case "getTrabajosProximosBoss": getTrabajosProximosBoss(obj, session); break;
            case "getMisTrabajos": getMisTrabajos(obj, session); break;
            case "getMisTrabajosEntreFechas": getMisTrabajosEntreFechas(obj, session); break;
            case "getMisTrabajadores": getMisTrabajadores(obj, session); break;
            case "reporteHorasCliente": reporteHorasCliente(obj, session); break;
    ...
    public static void isJefe(JSONObject obj, SSSessionAbstract session) {
        try {
            String consulta = "select is_jefe('" + obj.getString("key_evento") + "', '"+obj.getString("key_usuario")+"') as json";
            JSONObject data = SPGConect.ejecutarConsultaObject(consulta);
            obj.put("data", true);
    ...
    public static void editarClock(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            if(!data.has("fecha_ingreso") || data.isNull("fecha_ingreso") || data.optString("fecha_ingreso").length()==0){
                SPGConect.ejecutar("update "+COMPONENT+" set fecha_ingreso = null   where key = '"+data.getString("key")+"'");
                data.remove("fecha_ingreso");
            }

            if(!data.has("fecha_salida") || data.isNull("fecha_salida") || data.optString("fecha_salida").length()==0){
                SPGConect.ejecutar("update "+COMPONENT+" set fecha_salida = null   where key = '"+data.getString("key")+"'");
    ...
    public static void aceptarInvitacion(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            JSONObject staffUsuario = SPGConect.ejecutarConsultaObject("select get_staff_usuario_entre_fechas('"+staffUSuario.getString("key_usuario")+"', '"+staff.getString("fecha_inicio")+"', '"+fecha_fin+"') as json");
    ...
    public static void getMisTrabajosEntreFechas(JSONObject obj, SSSessionAbstract session) {
        try {
            String consulta = "select get_mis_trabajos('"+obj.getString("key_usuario")+"', '"+obj.getString("fecha_inicio")+"', '"+obj.getString("fecha_fin")+"') as json";
            JSONObject data = SPGConect.ejecutarConsultaObject(consulta);
            obj.put("data", data);
            obj.put("estado", "exito");
        } catch (Exception e) {
    ...
    public static void getMisTrabajadores(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            if(obj.has("fecha")){
                consulta = "select get_mis_trabajadores('"+obj.getString("key_usuario")+"', '"+obj.getString("fecha")+"') as json";
            }
    ...
    public static void reporteHorasCliente(JSONObject obj, SSSessionAbstract session) {
        try {
            String consulta = "select get_trabajadores_cliente('"+obj.getString("key_cliente")+"', '"+obj.getString("fecha_inicio")+"', '"+obj.getString("fecha_fin")+"') as json";
